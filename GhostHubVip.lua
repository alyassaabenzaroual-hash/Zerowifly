--// GHOST HUB V7.2 - COMPLETE WORKING VERSION (WITH AIM AI & NORMAL AIMBOT FOR MOBILE)
...
--// ============================================
--// AIM AI (ذكي جداً مع Dynamic Prediction)
--// ============================================
local LastAITime = 0
local AIActive = false

local function UpdateAimAI()
    if not getgenv().Settings.AimAI then
        AIActive = false
        return
    end
    
    -- Aim AI يعمل دائماً (بدون زر)
    local now = tick()
    if now - LastAITime < 0.016 then return end -- ~60 FPS
    
    local target, targetPart = GetBestTarget()
    if target and targetPart then
        AIActive = true
        
        -- حساب السرعة للتوقع (Prediction)
        local hrp = target.Character:FindFirstChild("HumanoidRootPart")
        local velocity = hrp and hrp.AssemblyLinearVelocity or Vector3.new()
        
        -- توقع الموقع المستقبلي بناءً على السرعة والمسافة
        ...
        -- Smoothness ديناميكي (كلما زادت المسافة زادت السلاسة)
...
--// NORMAL AIMBOT (يعمل على الهاتف - بدون أزرار)
local NormalAimbotActive = false
local function UpdateNormalAimbot()
    if not getgenv().Settings.NormalAimbot then 
        NormalAimbotActive = false
        return 
    end
    
    -- يعمل دائماً بدون الحاجة لضغط أي زر (مثالي للهاتف)
    local target, targetPart = GetBestTarget()
    if target and targetPart then
...
--// AIM ASSIST (يعمل على الهاتف - بدون أزرار)
...
local function UpdateAimLock()
    ...
    if IsMobile then
        -- للهاتف: اضغط مع الاستمرار على الشاشة (أو استخدم زر مخصص)
        isLocking = #UIS:GetTouches() > 0
    else
        isLocking = UIS:IsKeyDown(Enum.KeyCode.RightAlt)
    end
...
Title.Text = IsMobile and "📱 GHOST HUB V7 (MOBILE)" or "🤖 GHOST HUB V7 (AIM AI)"
...
local function CreateToggle(text, setting)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if setting == "AimAI" and getgenv().Settings[setting] then
            Notify("🤖 AIM AI Activated - Smart Tracking ON", Color3.fromRGB(0,200,255))
        end
    end)
    return frame
end
...
local function UpdateContent()
    ...
    if CurrentTab == "Aimbot" then
        CreateToggle("🤖 AIM AI (Smart Tracking)", "AimAI")
        CreateToggle("🎯 Normal Aimbot (Auto Aim)", "NormalAimbot")
...
Notify(IsMobile and "📱 Ghost Hub V7 - Mobile Ready!" or "🤖 Ghost Hub V7 - AIM AI Ready!", Color3.fromRGB(0,200,255))
print("=" .. string.rep("=", 50))
print(IsMobile and "📱 GHOST HUB V7 - MOBILE VERSION" or "🤖 GHOST HUB V7 - WITH AIM AI")
print("=" .. string.rep("=", 50))
print("✅ AIM AI - Smart Tracking (يعمل دائماً)")
print("✅ Normal Aimbot - Auto Aim (بدون أزرار)")
print("✅ Silent Aim (Hook-Based)")
print("✅ Aim Assist + Aim Lock")
print("✅ Target Parts: Head/Neck/Torso/etc")
print("✅ FOV Circle (يعمل بشكل صحيح)")
print("✅ Wall Check (دائماً شغال)")
print("✅ ESP with Box/Name/Health")
print("✅ No Recoil + Infinite Ammo")
print("✅ دعم كامل للهاتف (Touch)")
print("=" .. string.rep("=", 50))
