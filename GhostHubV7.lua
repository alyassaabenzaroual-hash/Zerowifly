...
local SilentAimManager = {
    ...
    HookRaycast = function()
        ...
        workspace.Raycast = function(origin, direction, params)
            ...
            if getgenv().Settings.SilentAim and TargetSelector and TargetSelector.CurrentTarget then
                local target = TargetSelector.CurrentTarget
                if target and target.Hrp and target.Character then
                    ...
                    local fovRad = math.rad(getgenv().Settings.FOV / 2)
                    if dotProduct >= math.cos(fovRad) then
                        -- Calculate if ray would hit target
                        local toTarget = targetPos - origin
                        local distanceToTarget = toTarget.Magnitude
                        local maxDistance = (direction * 1000).Magnitude
                        
                        if distanceToTarget <= maxDistance then
                            local targetHitPart = target.Character:FindFirstChild(getgenv().Settings.TargetPart) or target.Hrp
                            if targetHitPart then
                                return targetHitPart, targetPos, targetHitPart.Material
                            end
                        end
                    end
                end
...
local function Initialize()
    safeCall(function()
        ...
        NotificationSystem.Show("👻 Ghost Hub V7.2 Complete!", 3, "success")
